    ...
    <div style={{ marginTop: 20 }}>
      {files.length === 0 ? (
        <div style={{ textAlign: "center", padding: "60px 20px", color: COLORS.textDim }}>
          ...
          <p style={{ margin: "16px 0 0", fontSize: 14 }}>No files yet. Upload some PDFs to get started.</p>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {files.map((f, i) => <FileCard key={i} file={f} onView={onView} onRemove={() => onRemove(f)} onDownload={() => {}} />)}
        </div>
      )}
    </div>
  </div>
...
export default function PDFMasterApp() {
  ...
  const addFiles = useCallback((newFiles) => {
    const mapped = newFiles.map(f => ({
      name: f.name, size: f.size, pages: Math.floor(Math.random() * 30) + 1, modified: "Just now", raw: f,
    }));
